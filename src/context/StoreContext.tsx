...
import { getSizeAdjustedPrice } from "@/lib/priceUtils";
import { 
  getProductsAction, 
  registerUserAction, 
  loginUserAction, 
...
export interface CartItem {
  ...
  quantity: number;
  selectedSize: string;
  /** Effective unit price after size-based adjustment */
...
export function StoreProvider({ children }: { children: React.ReactNode }) {
  ...
  const [paymentSettings, setPaymentSettings] = useState<{ upiId: string; merchantName: string; qrCodeUrl: string | null }>({
    upiId: "admin@lunarthrift",
    merchantName: "Lunar Thrift Co.",
    qrCodeUrl: null
  });
    ...
    if (!savedDb) {
      const initialUsers = [
        ...
        },
        {
          id: "usr-prince",
          name: "Prince Patel",
          email: "24cs071@charusat.edu.in",
          password: "prince@@0000",
          role: "USER",
          isFirstOrder: true
        }
      ];
  ...
  const addToCart = (product: Product, quantity: number, size: string) => {
    ...
    const adjustedPrice = getSizeAdjustedPrice(product.price, size);
    setCart((prev) => {
      ...
      if (existingIndex > -1) {
        ...
        return [...prev, { product, quantity, selectedSize: size, adjustedPrice }];
      }
    });
  };
