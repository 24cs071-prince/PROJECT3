...
import { getSizePriceBadge } from "@/lib/priceUtils";
...
export default function CartDrawer() {
  ...
  const subtotal = cart.reduce((total, item) => total + (item.adjustedPrice ?? item.product.price) * item.quantity, 0);
                    ...
                    <div className="flex-grow flex flex-col justify-between">
                      <div className="flex justify-between items-start gap-1">
                        <div>
                          ...
                          {/* Size surcharge badge */}
                          {(() => {
                            const badge = getSizePriceBadge(item.product.price, item.selectedSize);
                            if (!badge) return null;
                            const isMarkup = badge.startsWith("+");
                            return (
                              <span className={`inline-block text-[8px] font-bold uppercase tracking-widest px-1.5 py-0.5 rounded mt-1 ${
                                isMarkup
                      ...
                      <div className="flex items-center justify-between mt-3">
                        ...
                        <span className="text-xs font-bold text-[#F0EFE7]">
                          ${((item.adjustedPrice ?? item.product.price) * item.quantity).toFixed(2)}
                        </span>
                      </div>
                    </div>
                  </div>
