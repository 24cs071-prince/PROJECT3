...
import { getSizeAdjustedPrice, getSizePriceBadge, getSizePriceDiff } from "@/lib/priceUtils";
      ...
      <div className="max-w-7xl mx-auto px-6 py-12 flex-grow w-full">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16">
          ...
          <div className="flex flex-col gap-6">
            ...
            {/* Live Size-Adjusted Price Block */}
            {(() => {
                ...
                <div className="flex flex-wrap gap-3">
                  {(() => {
                    ...
                    return availableSizes.map((sz) => {
                      ...
                      const badge = getSizePriceBadge(product.price, sz);
                      const diff = getSizePriceDiff(product.price, sz);
                      return (
                        <button
                          key={sz}
                          ...
                          className={`w-14 min-h-[52px] rounded-lg flex flex-col items-center justify-center gap-0.5 border transition-all text-xs font-bold relative uppercase px-2
                            ${isAvailable 
                              ? isSelected
                        ...
                        >
                          <span>{sz}</span>
                          {/* Price diff badge on each size */}
                          {isAvailable && badge && (
                            <span className={`text-[7px] font-bold tracking-tighter leading-none ${
                              diff > 0 ? "text-amber-400" : "text-green-400"
                            }`}>
                              {badge}
                            </span>
                          )}
                          {isAvailable && !badge && (
                            <span className="text-[7px] text-[#B8A98F] font-semibold tracking-tighter">
                              BASE
                            </span>
                          )}
                          {!isAvailable && product.stock === 0 ? (
                            <span className="text-[6px] text-red-400 font-semibold scale-75 tracking-tighter">SOLD OUT</span>
                          ) : !isAvailable ? (
